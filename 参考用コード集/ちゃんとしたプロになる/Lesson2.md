...
このやり方では**最低でも２つ**、タブレットも含めるなら**３つ**のデザイン・HTMLを作成することになり、大変なコストがかかった。  
...
以前はデスクトップ用のサイトを最初に制作し、モバイル用にそれを１カラムで表示させる、というのが主流だった。（デスクトップファースト）  
...
## ブレイクポイントの考え方 
ブレイクポイントの数値をいくつで設定するか、という部分に絶対的な正解はなく、  
...
タブレットとPCの境界にあたる「960px」の**２つ**をブレークポイントとする例などがある。  

スマートフォンの新機種が登場すれば、表示幅の主流やシェアは移り変わるし、どの年代までをカバーするかにもよって変わってくる。  
なのでブレークポイントに絶対は存在しない、というのを覚えておく。  
>  【ポイント】  
...
また、タブレット表示をモバイルファーストで設計するか、デスクトップファーストで設計するかも検討する必要がある。  
...
逆にテキストを表示する部分は、横幅の最大値を決めるほうが、１行あたりの文字数が増えすぎず読みやすい文章になる。

![設計例](images/d1549eb7-4167-4dd9-a91b-2621766822c6-0.jpg)  

横幅を可変とする箇所は、画像に対して文字が小さくなりすぎないように、**font-size**（文字サイズ）を検討しよう。  

一般的には、表示幅が変化しても文字の大きさは同じままにしたほうが見やすいことが多いが、そこも柔軟に対応していく。  
