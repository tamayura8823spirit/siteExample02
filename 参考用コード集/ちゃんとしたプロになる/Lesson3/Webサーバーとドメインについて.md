...
「.jp」ドメインは**属性型・地域型JPドメイン名、都道府県型JPドメイン名、汎用JPドメイン名**の３つに分類された SLD を取得できる。  
これは日本の組織に対する種別を表すもので、たとえば株式会社のサイトで利用される「**.co.jp**」や学校法人を表す「**ac.jp**」、  
政府機関の「**go.jp**」などがある。  
これらは組織が実在する証明書を提出する必要があるが、  
特に組織を種別化する必要がなければ、汎用JPドメイン名として「**example.jp**」という形での取得も可能。  

![jpdomein](images/601a8470-202b-4ef0-9a01-32957969a1ab-0.jpg)
...
## DNSサービスの選び方
多くのホスティングサービスでは、サーバーをレンタルした際に、そのホスティング会社が所持している**サブドメイン**が提供される。  
その提供されたサブドメインでもWebサイトの運用はできるが、  
**オリジナル性やSEOの観点から独自ドメインの取得が推奨されている**。  

**DNSサービスが止まってしまうとWebサイトも見れなくなってしまう**ので、安定稼働しているというのが重要な選択基準となる。  
また、使用したいTLDがある場合は、**そのドメイン名を取り扱っているサービス会社を選び必要があるため**、  
サイトの運用目的に合わせたDNSサービスを検討しよう。  

>  【 **memo** 】  
> 独自ドメインはサーバー会社が提供するDNSサービスを利用することもできるし、  
> 別会社のDNSサービスを利用することも可能。  
