...
## polyfillを導入する
**polyfill** とはブラウザに後方互換を持たせるライブラリのことで、  
`IntersetionObserver()` というのは割と新しいクラスなので、  
このクラスが使えない古いブラウザでも同じように動作させるために、このライブラリをインポートして補完させることができる。  

たとえば `IntersetionObserver()` の polyfill でいえば、  
**Github**上にGoogle社が作った polyfill が載っているので、  
そのファイルを読み込んでやれば、機能が働くことになる。  

<br>

まずは「index.html」内でファイルを読み込んであげて、  
その polyfill を使うときには `this.io.POLL_INTERVAL` というプロパティに対して、  
間隔を **ms** で設定してあげる。  
